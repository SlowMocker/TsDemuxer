...
int ts::demuxer::demux_ts_packet(const char* ptr, double* video_fps)
{
    ...
    u_int16_t pid=to_int(ptr+1); // 从 packet header 中截取同步字节后的数据
    u_int8_t flags=to_byte(ptr+3);

    // packet header 信息
    bool transport_error=pid&0x8000; // 错误指示信息（1: 该包至少有 1 bits 传输错误）
...
//    prefix.clear();
    ...
    if(prefix.length() && prefix[prefix.size()-1] != '.')
        prefix+='.';
    ...
    for(u_int64_t pn=1;;pn++)
    {
        ...
        }else
        {
            if(file.read(buf,188)!=188)
                break;
            // TS 包大小为 188 字节（header 4 字节，playload 184 字节）【或者 204 字节（在 188 字节后面添加 16 字节的 CRC 校验数据）】
            // TS 包 Header
            // TS packet header 同步字节（首字节） 固定为 0x47
            // 校验 TS 包格式
